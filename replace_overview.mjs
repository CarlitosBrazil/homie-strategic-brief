...
const file = 'src/components/ResearchDashboard.tsx';
const raw = readFileSync(file, 'utf-8');

// Detect line ending
const eol = raw.includes('\r\n') ? '\r\n' : '\n';

// Split on the actual line ending so lines are clean (no trailing \r)
const lines = raw.split(eol);

console.log(`Original file: ${lines.length} lines, EOL: ${eol === '\r\n' ? 'CRLF' : 'LF'}`);

// Find OverviewView start and MarketView start
const startIdx = lines.findIndex(l => l.includes('function OverviewView'));
const endIdx = lines.findIndex((l, i) => i > startIdx && /^function MarketView/.test(l.trim()));
...
console.log(`OverviewView: line ${startIdx + 1}, MarketView: line ${endIdx + 1}`);
...
const after = lines.slice(endIdx); // starts at "function MarketView..."

const result = [...before, ...newOverview, '', ...after];
...
const verifyLines = verify.split(eol);
console.log(`New file: ${verifyLines.length} lines`);
console.log(`Line ${startIdx + 1}: ${verifyLines[startIdx].substring(0, 50)}`);
const newMarket = verifyLines.findIndex(l => l.includes('function MarketView'));
console.log(`MarketView now at line: ${newMarket + 1}`);
console.log(`Has double CR: ${verify.includes('\\r\\r')}`);
console.log('✅ Done');
